...
void OscillaTroll02AudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    asamplesPerBlock=samplesPerBlock;
...
juce::AudioProcessorValueTreeState::ParameterLayout OscillaTroll02AudioProcessor::createParams()
{
    ...
    //Glide
        params.push_back(std::make_unique<juce::AudioParameterChoice>("MORG", "Multi-touch or Glider", juce::StringArray {"Multi-touch", "Glider"}, 0));
        params.push_back(std::make_unique<juce::AudioParameterInt>("GLIDE", "Glide",  0, 100000, 10000 ));
    
    //LFO type
        params.push_back(std::make_unique<juce::AudioParameterChoice>("HORR", "Hertz or Rate", juce::StringArray {"Hertz", "Rate"}, 0));

    return {params.begin(), params.end()};
    
}

void OscillaTroll02AudioProcessor::refreshSynthesiser(bool multi)
{
    synth.clearSounds();
    synth.clearVoices();
